        ...
        <div className={styles.requestList}>
          {members.map((member) => (
            <article key={member.member_number} className={`${styles.requestRow} ${styles.memberRow}`}>
              ...
              <div><span className={styles.label}>Quyền / Ngày tham gia</span><strong>{member.role}</strong><small>{formatDate(member.created_at)}</small><Link href={`/admin/members/${member.id}`} className={styles.editMemberLink}><SquarePen size={14} aria-hidden="true" /><LocalizedText vi="Chỉnh sửa" en="Edit" /></Link></div>
            </article>
          ))}
        </div>
      )}
