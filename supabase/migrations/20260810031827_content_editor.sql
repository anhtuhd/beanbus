create table public.content_change_history (
...
create index content_change_history_content_idx
on public.content_change_history (content_type, content_id, created_at desc);

alter table public.content_change_history enable row level security;
revoke all on table public.content_change_history from anon, authenticated;
grant select on table public.content_change_history to authenticated;
grant all on table public.content_change_history to service_role;

create policy "Admins read content change history"
on public.content_change_history for select to authenticated
...
begin
  ...
  if v_event_id is null then
    v_event_id := 'event-' || encode(gen_random_bytes(8), 'hex');
  else
...
revoke all on function public.admin_upsert_event(text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, text, integer, boolean, boolean, integer) from public;
grant execute on function public.admin_upsert_event(text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, text, integer, boolean, boolean, integer) to authenticated;

create function public.admin_upsert_blog_post(
  p_post_id text,
  p_slug text,
  p_title_vi text,
  p_title_en text,
...
begin
  ...
  if v_post_id is null then
    v_post_id := 'post-' || encode(gen_random_bytes(8), 'hex');
  else
...
revoke all on function public.admin_upsert_blog_post(text, text, text, text, text, text, text, text, text, text, text, text, text, text, boolean, integer) from public;
grant execute on function public.admin_upsert_blog_post(text, text, text, text, text, text, text, text, text, text, text, text, text, text, boolean, integer) to authenticated;
