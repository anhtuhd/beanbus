...
3. **Google happy path chưa được kiểm thử và production deployment còn stale.** E2E local chứng minh Google-only UI; hosted authorize trả `302`, nhưng chưa chứng minh Gmail mới tạo `auth.users`, `profiles`, session và logout đúng. Build production hiện vẫn render phone form cũ và cần redeploy.
