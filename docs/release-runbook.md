...
6. Verify `GET /api/health` returns `200` with `{"status":"ok"}`, a usable `x-request-id`, and (on Vercel) a 12-character `revision` matching the deployed commit SHA prefix. The 2026-08-12 check returned `mode: production` with revision `5a8a6f31771a`.
