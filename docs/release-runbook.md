...
5. Deploy the application with the production environment variables above.
   Current read-only check (2026-08-11): production flags expose Google enabled and Phone disabled, but the deployed HTML still contains the legacy disabled Zalo form. Do not sign off the Google-only UI until the current source is committed and redeployed.
