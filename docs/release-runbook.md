...
5. Deploy the application with the production environment variables above. Production provider contexts skip demo fixture hydration and browser persistence; server-owned Supabase data remains authoritative. The latest CI-only hardening commit `acfd2c5` does not require a Vercel redeploy; the verified production revision remains `384f095b3827`.
