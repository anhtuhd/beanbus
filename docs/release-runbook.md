...
3. Apply the reviewed migrations with `npm run db:push` against the linked hosted project.
   Current hosted record (2026-08-11): the remote inventory contains 37 applied migrations through `20260811050000_fix_flash_sale_error_precedence.sql`. The new `20260811120000_fix_loyalty_redemption_collision.sql` must pass CI before it is applied. A read-only verification confirmed the flash-sale function prioritizes the member limit, the SePay reconciliation ledger and voucher reservation ledger exist, and both Zalo cron names have zero active jobs. Remote `db lint --fail-on error` passes with no schema errors. The CLI emitted a Docker catalog-cache warning after a successful remote apply; it does not invalidate the migration result.
