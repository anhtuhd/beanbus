...
5. Deploy the application with the production environment variables above.
   Current read-only check (2026-08-11): production flags expose Google enabled and Phone disabled, but the deployed HTML still contains the legacy disabled Zalo form. Source commits through `74571c7` are pushed on `codex/zalo-otp-integration`; do not sign off the Google-only UI until Vercel redeploys the branch or merges it into the production branch.
